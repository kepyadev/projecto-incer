...
const UpdateEquipamentoTypeScreen = () => {
  const { errors, register, handleSubmit } = useForm({
    resolver: yupResolver(validationSchema),
  });
  const id = localStorage.getItem('active_item');
  const [animalType, setMachineType] = useState<IMachineType>();
  const { snackMessage, setSnackMessage } = useAsyncState();

  useEffect(() => {
    if (id)
      getOneEquipamentoType(id)
        .then(response => {
  ...
  const onSubmit = handleSubmit(data => {
    ...
    updateEquipamentoType(newMachineType, animalType[MachineType.Id])
      .then(() => {
        setSnackMessage({
          isError: false,
        ...
        <Grid container spacing={2}>
          <Grid item md={12} sm={12} xs={12}>
            <TextField
              ...
              defaultValue={animalType[MachineType.Description]}
            />
          </Grid>
...
export default UpdateEquipamentoTypeScreen;
